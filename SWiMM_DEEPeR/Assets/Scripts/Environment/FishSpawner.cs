...
public class FishSpawner : MonoBehaviour
{
    ...
    public GameObject m_group_parent;
    ...
    {
        spawn_centre = rov.transform.position;
        ...
        m_group_parent.transform.parent = transform;

        GetWaypoints();
        InitialiseGroups();
        SpawnAllNPCs();
        SimulationManager._instance.event_master.ai_groups_complete_event.Raise();
    }
            ...
            if (group.objectPrefab != null && group.enableSpawner)
            {
                ...
                if (temp_group != null && temp_group.GetComponentInChildren<Transform>().childCount < group.maxAmount)
                {
                    for (int i = 0; i < group.maxAmount; ++i)
                    {
                        ...
                        temp_spawn.GetComponent<FishMovement>().random_movement = group.randomMovement;
                        temp_spawn.GetComponent<FishMovement>().ai_manager = this;
                        temp_spawn.GetComponent<FishMovement>().rotation_offset = temp_spawn.transform.rotation.eulerAngles;
                        temp_spawn.GetComponent<FishMovement>().m_mix_max_speed = new Tuple<float, float>(group.minSpeed, group.maxSpeed);
    ...
    public Vector3 GetRandomValidPosition(Vector3 valid_directions)
    {
        ...
        {
            random_directions.x += Random.Range(-spawn_radius, spawn_radius);
        }
        ...
        if (valid_directions.y > 0)
        {
            ...
            random_directions.y += Random.Range(-spawn_radius, spawn_radius);
        }

        if (valid_directions.z > 0)
        {
            random_directions.z += Random.Range(-spawn_radius, spawn_radius);
        }
