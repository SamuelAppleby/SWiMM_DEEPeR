"""
WORLD MODELS VAE ARCHITECTURE - STRAIGHTFORWARD CONV AND DECONV, NO RESNET
"""

import tensorflow as tf
from cmvae_models import world_models
import numpy as np
...
class WmvaeDirect(object):
    ...
    def _build_graph(self):
        ...
        with self.graph.as_default():
            ...
            # Encoder
            # self.mu, self.logvar = self.q_img(self.img_gt)

            # VAE
            # self.stddev = tf.math.exp(self.logvar / 2.0)
            # self.epsilon = random_normal([self.batch_size, self.z_size])
            # self.z = self.mu + self.stddev * self.epsilon
            ...
            if self.is_training:
                ...
                self.total_loss = self.img_loss + self.beta * self.kl_loss
    ...
    def decode(self, z):
        ...
        """
        return self.sess.run(self.img_recon, feed_dict={self.z: z})
