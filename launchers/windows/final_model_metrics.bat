...
@REM Come back tomorrow to fix this otherwise my poor laptop might soon become scrap metal
for %%x in (%algorithms%) do (
    ...
    for %%s in (%seeds%) do (
        for %%r in (%render%) do (
            python inference.py --seed %%s --algorithm !n! --render %%r --pre_trained_model_path !w!
